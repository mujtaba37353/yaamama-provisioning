#!/bin/bash
# =============================================================================
# Seed a single warm pool slot by number
# Usage: bash seed-single-slot.sh <slot_number>
# Example: bash seed-single-slot.sh 06
# =============================================================================

set -euo pipefail

SLOT_NUM="${1:?Usage: seed-single-slot.sh <slot_number>}"
SLOT_NUM=$(printf "%02d" "$((10#$SLOT_NUM))")
...
if [ ! -d "$SKELETON_DIR" ] || [ ! -f "$SKELETON_SQL" ]; then
  echo "Error: WP skeleton not found."
  exit 1
fi

if [ -d "$SLOT_PATH" ]; then
  echo "Slot ${SLOT_NAME} already exists, skipping."
  exit 0
fi
